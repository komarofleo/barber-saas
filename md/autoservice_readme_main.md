...
**AutoService** - это комплексное решение для автоматизации записи клиентов в автосервис на пр.Октября.
...
| Ручное распределение мастеров и постов | Автоматическое или ручное назначение с учетом загрузки |

### Ключевые метрики автосервиса

- **6-10 мастеров** одновременно
- **5-7 постов** для обслуживания
...
- ✅ **Подтверждение записей** с назначением мастера и поста
...
- 🗂️ **Справочники** (услуги, мастера, посты, клиенты)
...
docker exec autoservice_postgres pg_isready -U autoservice_user
```
...
Проект разработан для внутреннего использования автосервиса.
...
**Заказчик:** Автосервис на пр.Октября  
**Контакт:** autoservice@example.com