...
Полное описание REST API системы AutoService (FastAPI backend).
...
- `post` - конкретный пост (укажите `post_id`)
