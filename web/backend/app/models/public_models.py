...
- Company - компании (салоны красоты)
- Plan - тарифные планы
...
class Company(Base):
    """
    Модель компании (салона красоты) в public схеме.
    ...
    subscription_status = Column(String(20), default="pending", index=True)  # Исправлено: String вместо SQLEnum
...
class Plan(Base):
    ...
    Определяет ценовую политику и лимиты для салонов красоты.
    """
    __tablename__ = "plans"
    __table_args__ = {"schema": "public"}
    ...
    max_masters = Column(Integer, default=3)
...
class Subscription(Base):
    ...
    status = Column(String(20), default="active", index=True)  # Исправлено: String вместо SQLEnum
